...
WITH
    ...
    numb_sorted_by_valid_from_dttm AS
    (
        SELECT ROW_NUMBER() OVER() AS numb, *
        FROM sorted_by_valid_from_dttm
    ),
    numb_sorted_valid_to_dttm AS
    (
        SELECT ROW_NUMBER() OVER() AS numb, *
        FROM sorted_by_valid_to_dttm
    ),
    
    -- Таблица с атрибутами: id, valid_from_dttm, valid_to_dttm.
    valid_dttm AS
    (
        SELECT numb_sorted_by_valid_from_dttm.numb, numb_sorted_by_valid_from_dttm.id, valid_from_dttm, valid_to_dttm
        FROM numb_sorted_by_valid_from_dttm, numb_sorted_by_valid_to_dttm
        WHERE numb_sorted_by_valid_from_dttm.numb = numb_sorted_by_valid_to_dttm.numb
    ),
