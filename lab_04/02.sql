--Пользовательская агрегатная функция CLR для получения всех жителей, проживающих в замках n-го королевства
CREATE EXTENSION IF NOT EXISTS plpython3u;
DROP FUNCTION IF EXISTS GetCastlePopulation;
create or replace function GetCastlePopulation(n integer)
RETURNS DECIMAL
AS $$
	plan = plpy.prepare("select population from Castle where id_kingdom = $1;", ["INT"])
